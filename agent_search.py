...
from search.recombination import recombination
from search.module_evolution import evolution
from search.module_predictor import predict_performance
...
def run_benchmark(agent: Agent, n: int = 50) -> float:
    ...
    try:
        os.chdir('./search/alfworld')
        current_path = os.path.dirname(os.path.abspath(__file__))
        parent_path = os.path.dirname(current_path)
        if parent_path in sys.path:
            sys.path.remove(parent_path)
        sys.path.append(os.path.join(current_path, 'search/alfworld'))
        
        # import alfworld_run
        from search.alfworld import alfworld_run
        importlib.reload(alfworld_run)
        from search.alfworld.alfworld_run import run_alfworld
...
def remove_test_module(module_type: str) -> None:
    ...
    """
    module_file = f'search/alfworld/{module_type}_modules.py'
    ...
    start_idx = -1
    for i, line in enumerate(lines):  # 找到test模块的定义行
        if f'class {test_class}' in line:
            start_idx = i
            break
    
    if start_idx != -1:  # 把test模块定义前的代码保留，其他删除
...
def write_test_module(module_type: str, module_code: str) -> None:
    ...
    with open(module_file, 'a') as f:
        ...
        else:  # 将module_code原本的 testclass(*Base)替换为新的类名，但类中具体内容不变
            f.write('\n' + re.sub(
                r'class \w+\(.*Base\)',
        ...
        if module_name != 'None':
            ...
            for module in module_archive:
                if module['name'] == module_name:
                    write_test_module(module_type, module['code'])  # 写入module代码
...
def agent_search():
    ...
    try:
        # Load modules
        module_data = {
            'planning': load_modules_from_json('./search/planning_modules.json'),
            'reasoning': load_modules_from_json('./search/reasoning_modules.json'),
            'tooluse': load_modules_from_json('./search/tooluse_modules.json'),
            'memory': load_modules_from_json('./search/memory_modules.json')
        }
        ...
        for iteration in range(num_iterations):
            ...
            print("\n\n[Predicting Recombined Agent Performance]")
