...
from modules.planning_modules import *
from modules.reasoning_modules import *
from modules.tooluse_modules import *
from modules.memory_modules import *
...
module_name_dict={
    ...
    "reasoning":{
        ...
        "dilu":"ReasoningDILU",
        "io":"ReasoningIO",
        "selfrefine":"ReasoningSelfRefine",
        "selfreflectivetot":"ReasoningSelfReflectiveTOT",
    ...
    "tooluse":{
        "base":"TooluseBase",
        "anytool":"TooluseAnyTool",
        "io":"TooluseIO",
        "test":"TooluseTest",
        "toolbench":"TooluseToolBench",
        "toolbenchformer":"TooluseToolBenchFormer",
        "toolformer":"TooluseToolFormer"
    }
}
# Define types
...
def get_module_code(module_dict):
    ...
    prompt_answer += f"Planning module: Planning{planning}\n"
    if planning != 'none':
        planning_class = eval(f"{module_name_dict['planning'][planning]}")
        prompt_answer += get_class_code(planning_class) + "\n"
        
    # Reasoning module  
    reasoning = module_dict['reasoning'].lower()
    prompt_answer += f"Reasoning module: Reasoning{reasoning}\n"

    reasoning_class = eval(f"{module_name_dict['reasoning'][reasoning]}")
    prompt_answer += get_class_code(reasoning_class) + "\n"
    
    # Tooluse module
    tooluse = module_dict['tooluse'].lower()
    prompt_answer += f"Tooluse module: Tooluse{tooluse}\n"
    if tooluse != 'none':
        tooluse_class = eval(f"{module_name_dict['tooluse'][tooluse]}")
        prompt_answer += get_class_code(tooluse_class) + "\n"
        
    # Memory module
    memory = module_dict['memory'].lower() 
    prompt_answer += f"Memory module: Memory{memory}\n"
    if memory != 'none':
        memory_class = eval(f"{module_name_dict['memory'][memory]}")
