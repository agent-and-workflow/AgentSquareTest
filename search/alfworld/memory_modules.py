...
class MemoryBase:
    def __init__(self, llms_type, memory_type: str, agent_id: str) -> None:
        self.llm_type = llms_type[0]
        self.embedding = OpenAIEmbeddings()
        db_path = os.path.join('./db', memory_type, f'agent_{agent_id}')
...
class DualLayerMemory():
    ...
    def retriveMemory(self, query_scenario):
        ...
        detailed_trajectory = similarity_results[0][0].metadata['task_trajectory']
        
        # Summarize the trajectory for insights
        summary_prompt = f'''Please summarize the following task trajectory in a concise manner:
{detailed_trajectory}
Summary: '''
...
class MemorySummarizer():
    ...
    def retriveMemory(self, query_scenario):
        ...
        for result in similarity_results:
            trajectory = result[0].metadata['task_trajectory']
            prompt = f'Please summarize the following task resolution into actionable insights: {trajectory}'
            summary = llm_response(prompt=prompt, model=self.llm_type, temperature=0.1)
            summarized_experience.append(summary)
        
        return '\n'.join(summarized_experience)
    def addMemory(self, current_situation):
        # Extract task description
        task_name = re.search(r'Your task is to:\s*(.*?)\s*>', current_situation).group(1)
        
        # Create document with metadata
        summary_prompt = f'Please summarize the following task resolution trajectory: {current_situation}'
        trajectory_summary = llm_response(prompt=summary_prompt, model=self.llm_type, temperature=0.1)
        memory_doc = Document(
            page_content=trajectory_summary,
            metadata={
                "task_name": task_name,
...
class InsightfulMemory():
    ...
    def retriveMemory(self, query_scenario):
        ...
        # Find similar memory based on task name and actions
        similarity_results = self.scenario_memory.similarity_search_with_score(
            task_name, k=3)
            
        # Evaluate each memory's relevance based on actions taken
        relevant_memory = ''
        for result in similarity_results:
            trajectory = result[0].metadata['task_trajectory']
            actions = re.findall(r'\> (.*?)\n', trajectory)
            prompt = f'''Evaluate the relevance of these actions to the ongoing task:
Ongoing task:
{query_scenario}
Actions taken:
{', '.join(actions)}
Rate from 1 to 10: '''
            relevance_score = llm_response(prompt=prompt, model=self.llm_type, temperature=0.1)
            if relevance_score > 5:
                relevant_memory = trajectory
                break
        return relevant_memory
    
    def addMemory(self, current_situation):
        # Create a structured document with actions and rationale
        task_name = re.search(r'Your task is to:\s*(.*?)\s*>', current_situation).group(1)
        actions = re.findall(r'\> (.*?)\n', current_situation)
        rationale = ' '.join([f'Action: {action}, Reason: ...' for action in actions])
        memory_doc = Document(
            page_content=f'Task: {task_name}\nRationale: {rationale}',
            metadata={
                "task_name": task_name,
                "task_trajectory": current_situation
            }
